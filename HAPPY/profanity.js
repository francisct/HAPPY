...
function checkForProfanity(word){
    if (checkForNegativeNoun(word)){
        return provideCuteWord();
    }
    else if(checkForNegativeAdjective(word)){
        return providePositiveAdjective();
    }
    else return null;
}

function checkForNegativeNoun(word){
    var found = false;
    negativeNouns.forEach(function(noun){
        if (word.toUpperCase() === noun.toUpperCase()){
            found = true;
            return;
        }
    });
...
function checkForNegativeAdjective(word){
    var found = false;
...
function provideCuteWord(){
    return positiveNouns[Math.floor(Math.random()*positiveNouns.length)];
}
